...
vector<cId> kmeans::getClusters(int k) {
    ...
    //     bool cont = false;
    //     for (int j = 0; j < ptsize; j += dim) {
    //         minDistance = -1;
    //         for (int i = 0; i < k; i++) {
    //             dist = distanceVec(centroids[i], points, j);
    ...
    //         transform(sum.begin(), sum.end(), points.begin() + j, sum.begin(), std::plus<double>());
